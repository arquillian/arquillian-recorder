 ...
 */
public class WebcamVideoRecorderCreator {
    ...
    public void onWebcamRecorderExtensionConfigured(@Observes(precedence = -100) VideoExtensionConfigured event) {

        if (takenResourceRegister.get() == null) {
            this.takenResourceRegister.set(new TakenResourceRegister());
        }

        Recorder recorder = new WebcamVideoRecorder(takenResourceRegister.get());
        recorder.init((WebcamVideoConfiguration) configuration.get());
